...
class InferenceEvaluator:
   ...
   def infer(self, dataloader, batch_size):
      self.model.eval()
      images_saved = 0
      total_loss = 0
      final_total_ssim = 0
      final_total_psnr = 0
      images_to_save = 277  # Number of images to save during inference
         ...
         for batch in dataloader:
            ...
            for i in range(outputs_norm.shape[0]):
               ...
               if images_saved < images_to_save:
                     img_filename = os.path.join(self.save_dir, f'sample_image_{images_saved}.png')
                     input_img_filename = os.path.join(self.save_dir, f'ref_image_{images_saved}.png')
                     save_image(outputs[i], img_filename)
                     save_image(sparse_inputs[i], input_img_filename)
                     images_saved += 1
   ...
   def infer_fb(self, dataloader, batch_size):
      self.model.eval()
      images_saved = 0
      total_loss = 0
      final_total_ssim = 0
      final_total_psnr = 0
      images_to_save = 8649  # Number of images to save during inference
      ...
      self.model.train()
      return total_loss, final_total_psnr, final_total_ssim 

class NiftiInferenceEvaluator:
   def __init__(self, model, criterion_mse, criterion_psnr, criterion_ssim, save_dir='saved_images'):
      self.model = model
      self.criterion_mse = criterion_mse
      self.criterion_psnr = criterion_psnr
   ...
   def infer(self, dataloader):
      ...
      final_total_ssim = 0
      final_total_psnr = 0
      images_to_save = 2245  # Number of images to save during inference, vessel: 2245, brain: 277
      ...
      with torch.no_grad():
         for batch in dataloader:
            ...
            for i in range(outputs_norm.shape[0]):
               ...
               # Save images conditionally
               if images_saved < images_to_save:
                     img_filename = os.path.join(self.save_dir, f'sample_image_{images_saved}.nii')
